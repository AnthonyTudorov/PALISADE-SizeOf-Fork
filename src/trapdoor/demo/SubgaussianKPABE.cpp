...
//	shared_ptr<ILParams> ilParamsConsolidated(new ILParams(n, bigModulus, rootOfUnity));
	...
	// Precompuations for FTT
//	ChineseRemainderTransformFTT<NativeInteger, BigVector>::PreCompute(rootOfUnity, n, q);
...
bool TestDCRTVecDecompose(int32_t base, usint k, usint ringDimension, size_t type){
	...
	{
		TIC(t1);
		lbcrypto::PolyVec2BalDecom(ilParams, base, digitCount, matrixDecomposePoly, &psi);
		timeEval = TOC(t1);
	}
	else {
		int64_t digitCount = (long)ceil(log2(bigModulus.ConvertToDouble())/log2(base));
		...
		TIC(t1);
		InverseRingVector(sampler, ilParams, matrixDecomposePoly, &psi);
		timeEval = TOC(t1);
	}
