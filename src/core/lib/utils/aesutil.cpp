...
int AESUtil::KeyExpansion(unsigned int rk[], const unsigned char cipherKey[], int keyBytes)
{
    int i = 0;
    unsigned int temp;
...
int AESUtil::KeySchedule(unsigned int rk[], unsigned int rrk[], const unsigned char cipherKey[], int keyBytes)
{
    int Nr, i;

    Nr = KeyExpansion(rk, cipherKey, keyBytes);
...
void AESUtil::EncryptBlock(const unsigned char pt[], unsigned char ct[])
{
...
void AESUtil::DecryptBlock(const unsigned char ct[], unsigned char pt[])
{
    unsigned int s0, s1, s2, s3, t0, t1, t2, t3, v0, v1, v2, v3;
    const unsigned int *rk;
    int r;
...
AESUtil::AESUtil(unsigned char *pIV, unsigned char *pKey, unsigned int KeyLen)
{

    no_rounds = KeySchedule(encryption_key,  decryption_key, pKey, KeyLen);
...
void AESUtil::Encrypt(unsigned char *pData, unsigned char *pCipher, unsigned int DataLen,ModeOfOperation mode)
{
    if(mode==CTR){
    	unsigned int i,j,k;
    	    unsigned char counterblock[16];
    	    for (i = 0; i < DataLen; i += BLOCK_SIZE) {
    	    	EncryptBlock(counterblock, pCipher);
    	    	for(j=0;j<BLOCK_SIZE;j++){
    	        	*(pCipher+j) ^=*(pData+i + j);
    	        }
    	        pCipher += BLOCK_SIZE;
            for( k = 16; k > 0; k-- )
                if( ++counterblock[k - 1] != 0 )
                    break;
    	    }
    }
}

void AESUtil::Decrypt(unsigned char *pCipher, unsigned char *pData, unsigned int CipherLen,ModeOfOperation mode)
{
	if(mode==CTR){
	    	unsigned int i,j,k;
	    	    unsigned char counterblock[16];
	    	    for (i = 0; i < CipherLen; i += BLOCK_SIZE) {
	    	        EncryptBlock(counterblock, pData);
	    	        for(j=0;j<BLOCK_SIZE;j++){
	    	        	*(pData+j) ^=*(pCipher+i + j);
	    	        }
	    	        pData += BLOCK_SIZE;
