...
void testJson(
	...
	Serialized testMap1;
	if (tp->pk->Serialize(&testMap1, tp->ctx, "Enc")) {
	...
	Serialized testMap2;
	if (tp->sk->Serialize(&testMap2, tp->ctx, "Enc")) {
	...
	Serialized testMap3;
	if (testCiphertext.Serialize(&testMap3, tp->ctx, "Enc")) {
	...
	Serialized testMap4;
	if (tp->evalKey->Serialize(&testMap4, tp->ctx, "Pre")) {
	...
	Serialized testMap5;
	if (tp->newSK->Serialize(&testMap5, tp->ctx, "Pre")) {
	...
	Serialized testMap6;
	if (preCiphertext.Serialize(&testMap6, tp->ctx, "Pre")) {
